...
//	glActiveTexture(mTexTargets[WoodBoxTex]);
	...
	// Brick
	mTexTargets[BrickTex] = GL_TEXTURE3;

	if (!image.loadFromFile("media/brick.png"))
		return EXIT_FAILURE;

	glBindTexture(GL_TEXTURE_2D, mTex[BrickTex]);
	glTexStorage2D(GL_TEXTURE_2D, 8, GL_RGBA8, image.getSize().x, image.getSize().y);
	glTexSubImage2D(GL_TEXTURE_2D
					, 0
					, 0, 0
	...
	// Depth
	mTexTargets[DepthTex] = GL_TEXTURE4;
	glBindTexture(GL_TEXTURE_2D, mTex[DepthTex]);

	glTexImage2D(GL_TEXTURE_2D, 0, GL_DEPTH_COMPONENT32
					 , mDepthTexSizeX, mDepthTexSizeY
...
//	std::vector<glm::vec4> floorColours;
//	for(unsigned int i = 0; i < 4; i++)
//		floorColours.push_back(glm::vec4(0, 1, 0.25, 1));

	std::vector<glm::vec3> floorNorms;
	for(unsigned int i = 0; i < 4; i++)
		floorNorms.push_back(glm::vec3(0.f, 1.f, 0.f));
	...
	const GLushort floorEle[] =
	{
		0, 1, 2,
		0, 2, 3,
	};

	glBindBuffer(GL_ARRAY_BUFFER, mVBO[FloorVertBuf]);
//	glBufferData(GL_ARRAY_BUFFER, sizeof(planeVerts), &planeVerts[0], GL_STATIC_DRAW);
	glBufferData(GL_ARRAY_BUFFER, sizeof(floorVerts), &floorVerts[0], GL_STATIC_DRAW);
//	glBufferData(GL_ARRAY_BUFFER, 4 * sizeof(glm::vec4), &floorVerts[0], GL_STATIC_DRAW);

//   glBindBuffer(GL_ARRAY_BUFFER, mVBO[FloorColBuf]);
//   glBufferData(GL_ARRAY_BUFFER, floorColours.size() * sizeof(glm::vec4), &floorColours[0], GL_STATIC_DRAW);

   glBindBuffer(GL_ARRAY_BUFFER, mVBO[FloorUVBuf]);
   glBufferData(GL_ARRAY_BUFFER, sizeof(floorUV), &floorUV[0], GL_STATIC_DRAW);

	glBindBuffer(GL_ARRAY_BUFFER, mVBO[FloorNormBuf]);
   glBufferData(GL_ARRAY_BUFFER, floorNorms.size() * sizeof(glm::vec3), &floorNorms[0], GL_STATIC_DRAW);

	glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, mEBO[PlaneEBO]);
   glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(floorEle), &floorEle[0], GL_STATIC_DRAW);
...
void OGLRenderer::setupDepthPass()
{
	...
	glViewport(0, 0, mDepthTexSizeX, mDepthTexSizeY);
	glCullFace(GL_FRONT);
...
//	glm::mat4 V = getViewMatrix();
	...
	glm::mat4 P = mPlayerMatrices->getProjMatrix();
	glm::mat4 V = mPlayerMatrices->getViewMatrix();
...
OGLRenderer::WindowUPtr OGLRenderer::createWindow()
{
    ...
    mDepthTexSizeX = newWindow->getSize().x * 2;
    mDepthTexSizeY = newWindow->getSize().y * 2;
